...
describe("Integrated Cache Staleness", async function (this: Suite) {
  ...
  const dedicatedGatewayMaxAge = 1;
    ...
    plugins: [
      {
        ...
        plugin: async (context, next) => {
          if (
            ...
            assert.ok(
              typeof context.headers["x-ms-dedicatedgateway-max-age"] === "number",
              `${context.headers["x-ms-dedicatedgateway-max-age"]} = number`
            );
            assert.ok(
              context.headers["x-ms-dedicatedgateway-max-age"] !== 0,
              "x-ms-dedicatedgateway-max-age must be greater than zero."
            );
            assert.ok(
